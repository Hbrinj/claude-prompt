...
| `guard-push-review.sh` | `Bash` | Any `git push` touching the `feature/*` or `fix/*` namespaces without a review-evidence record matching HEAD. A push is guarded when **either** side of a refspec is `feature/*`/`fix/*`: the source being pushed (`origin feature/x`; `HEAD` resolved to the current branch; the current branch of the session cwd's repo — or a `git -C <dir>` override — for implicit pushes) or the destination being created (`origin chore/z:feature/sneaky`, `origin <sha>:feature/x` — rename pushes cannot smuggle unreviewed content in; force `+` and `refs/heads/` prefixes tolerated). The record lives at `<repo-root>/.claude/review-evidence/<branch-slug>.md` (slug = branch name with `/` → `-`), may be keyed by the source branch or, for rename/raw-sha pushes, the destination name, and must contain a line `HEAD: <full sha>` equal to the sha of the pushed content (`git rev-parse` of the source) — any new commit invalidates it; the workflow writes it at the review step (`skills/implement-feature.md` step 5). Skipped for: deletion pushes (`--delete`/`-d`, or an empty-source `:branch` refspec), pushes touching neither guarded namespace (main/master are `guard-push-main.sh`'s concern), and `--all`/`--mirror`/`--branches` pushes (`guard-push-main.sh` denies those outright). Same quote-stripping, shell-wrapper, and compound-command handling as `guard-push-main.sh`. |
